...
<div class="sections menu">
	<ul>
		<li class="section cover exclude" id="section_0" data-id="0"><div class="grouper">
			<div class="label"><a href="<?php echo nn::settings('DOMAIN').'/admin/publications/view/'.$publication->attr('id').'/0' ?>" data-ajax><?php echo Nn::babel('Cover') ?></a></div>
		</div></li>
	</ul>
	<ul id="1_sections" class="chapters sortable">
		<?php foreach($publication->nodes() as $node): ?>
			<li class="section" id="section_<?php echo $node->attr('id') ?>" data-id="<?php echo $node->section()->attr('id') ?>">
				<div class="grouper">
					<div class="label"><a href="<?php echo nn::settings('DOMAIN').'/admin/publications/view/'.$publication->attr('id').'/'.$node->section()->attr('id') ?>" data-ajax><?php echo $node->attr('title') ?></a></div>
					<div class="tools">
